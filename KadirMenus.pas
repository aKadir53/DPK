...
type
  TMenuItemKadir = class;

  EMenuError = class(Exception);
  TMenuKadir = class;
  TMenuBreak = (mbNone, mbBreak, mbBarBreak);
  TMenuChangeEvent = procedure (Sender: TObject; Source: TMenuItemKadir; Rebuild: Boolean) of object;
...
{ TMenuItemEnumerator }
  ...
  private
    FIndex: Integer;
    FMenuItem: TMenuItemKadir;
  public
    constructor Create(AMenuItem: TMenuItemKadir);
    function GetCurrent: TMenuItemKadir;
    function MoveNext: Boolean;
    property Current: TMenuItemKadir read GetCurrent;
  end;
...
{$IF DEFINED(CLR)}
  ...
  TMenuItemKadir = class(TComponent, IMenuItem)
{$ELSE}
  TMenuItemKadir = class(TComponent)
{$IFEND}
  private
    FCaption: string;
    FChecked: Boolean;
    ...
    FItems: TList;
    FShortCut: TShortCut;
    FParent: TMenuItemKadir;
    FMerged: TMenuItemKadir;
    FMergedWith: TMenuItemKadir;
    FMenu: TMenuKadir;
    ...
    procedure MergeWith(Menu: TMenuItemKadir);
    procedure RebuildHandle;
    procedure PopulateMenu;
    procedure SubItemChanged(Sender: TObject; Source: TMenuItemKadir; Rebuild: Boolean);
...
{$IFEND}
  protected
    ...
    function GetItem(Index: Integer): TMenuItemKadir;
    function GetMenuIndex: Integer;
    function GetAutoHotkeys: Boolean;
    function GetAutoLineReduction: Boolean;
    function InsertNewLine(ABefore: Boolean; AItem: TMenuItemKadir): Integer;
  ...
  public
    ...
    procedure Insert(Index: Integer; Item: TMenuItemKadir);
    procedure Delete(Index: Integer);
    procedure Clear;
    procedure Click; virtual;
    function Find(ACaption: string): TMenuItemKadir;
    function IndexOf(Item: TMenuItemKadir): Integer;
    function IsLine: Boolean;
    procedure GetChildren(Proc: TGetChildProc; Root: TComponent); override;
    function GetEnumerator: TMenuItemEnumerator;
    function GetImageList: TCustomImageList;
    function GetParentComponent: TComponent; override;
    function GetParentMenu: TMenuKadir;
    function HasParent: Boolean; override;
    function NewTopLine: Integer;
    function NewBottomLine: Integer;
    function InsertNewLineBefore(AItem: TMenuItemKadir): Integer;
    function InsertNewLineAfter(AItem: TMenuItemKadir): Integer;
    procedure Add(Item: TMenuItemKadir); overload;
    procedure Add(const AItems: array of TMenuItemKadir); overload;
    procedure Remove(Item: TMenuItemKadir);
    ...
    property Items[Index: Integer]: TMenuItemKadir read GetItem; default;
    property MenuIndex: Integer read GetMenuIndex write SetMenuIndex;
{$IF DEFINED(CLR)}
    property Parent: TMenuItemKadir read GetParent;
{$ELSE}
    property Parent: TMenuItemKadir read FParent;
{$IFEND}
  published
  ...
  [RootDesignerSerializerAttribute('', '', False)]
  TMenuKadir = class(TComponent)
  private
    FBiDiMode: TBiDiMode;
    FItems: TMenuItemKadir;
    FWindowHandle: HWND;
    FOwnerDraw: Boolean;
...
{$IFEND}
  protected
    ...
    procedure DoChange(Source: TMenuItemKadir; Rebuild: Boolean); virtual;
    procedure DoBiDiModeChanged;
    function GetHandle: HMENU; virtual;
    function IsOwnerDraw: Boolean;
    procedure Loaded; override;
    procedure MenuChanged(Sender: TObject; Source: TMenuItemKadir; Rebuild: Boolean); virtual;
...
{$IFEND}
  ...
  public
    constructor Create(AOwner: TComponent); override;
    function CreateMenuItem: TMenuItemKadir; dynamic;
    destructor Destroy; override;
    function DispatchCommand(ACommand: Word): Boolean;
    function DispatchPopup(AHandle: HMENU): Boolean;
    function FindItem(Value: NativeUInt; Kind: TFindItemKind): TMenuItemKadir;
    procedure GetChildren(Proc: TGetChildProc; Root: TComponent); override;
...
{$IFEND}
  ...
  published
    property Items: TMenuItemKadir read FItems;
  end;

  TMainMenu = class(TMenuKadir)
  private
    FOle2Menu: HMENU;
    FAutoMerge: Boolean;
  ...
  protected
    procedure MenuChanged(Sender: TObject; Source: TMenuItemKadir; Rebuild: Boolean); override;
  ...
  TPopupMenuKadir = class(TMenuKadir)
  private
    FPopupPoint: TPoint;
  ...
  public
    ...
    procedure Add(Popup: TPopupMenuKadir);
    procedure Remove(Popup: TPopupMenuKadir);
  end;

{$IF NOT DEFINED(CLR)}
  PMenuItem = ^TMenuItemKadir;
{$IFEND}

  TMenuItemStack = class(TStack)
  public
    procedure ClearItem(AItem: TMenuItemKadir);
  end;
...
function NewMenu(Owner: TComponent; const AName: string; const Items: array of TMenuItemKadir): TMainMenu;
function NewPopupMenu(Owner: TComponent; const AName: string;
  Alignment: TPopupAlignment; AutoPopup: Boolean; const Items: array of TMenuItemKadir): TPopupMenuKadir;
function NewSubMenu(const ACaption: string; hCtx: THelpContext;
  const AName: string; const Items: array of TMenuItemKadir; AEnabled: Boolean = True): TMenuItemKadir;
function NewItem(const ACaption: string; AShortCut: TShortCut;
  ...
  const AName: string): TMenuItemKadir;
function NewLine: TMenuItemKadir;

procedure DrawMenuItem(MenuItem: TMenuItemKadir; ACanvas: TCanvas; ARect: TRect;
...
type
  TIterator = function (MenuItem: TMenuItemKadir): Boolean{$IFDEF ITERATOR_OBJECTS} of object{$ENDIF};

procedure IterateMenus(Func: TIterator; Menu1, Menu2: TMenuItemKadir);
var
  IIndex: Integer;

  function Iterate(var I: Integer; MenuItem: TMenuItemKadir; AFunc: TIterator): Boolean;
  var
    Item: TMenuItemKadir;
  begin
    Result := False;
...
begin
  FClient := AClient as TMenuItemKadir;
end;
...
constructor TMenuItemEnumerator.Create(AMenuItem: TMenuItemKadir);
begin
  inherited Create;
  FIndex := -1;
  FMenuItem := AMenuItem;
...
function TMenuItemEnumerator.GetCurrent: TMenuItemKadir;
begin
...
constructor TMenuItemKadir.Create(AOwner: TComponent);
begin
...
destructor TMenuItemKadir.Destroy;
begin
...
procedure TMenuItemKadir.AppendTo(Menu: HMENU; ARightToLeft: Boolean);
const
...
procedure TMenuItemKadir.PopulateMenu;
var
{$IFDEF ITERATOR_OBJECTS}
...
{$ELSE !ITERATOR_OBJECTS}
  ...
  function AddIn(MenuItem: TMenuItemKadir): Boolean;
  begin
    MenuItem.AppendTo(FHandle, MenuRightToLeft);
    Result := False;
  end;
...
procedure TMenuItemKadir.ReadShortCutText(Reader: TReader);
begin
  ShortCut := TextToShortCut(Reader.ReadString);
end;

procedure TMenuItemKadir.MergeWith(Menu: TMenuItemKadir);
begin
...
procedure TMenuItemKadir.Loaded;
begin
  inherited Loaded;
...
procedure TMenuItemKadir.RebuildHandle;
var
  I: Integer;
  LRepopulate: Boolean;
begin
...
procedure TMenuItemKadir.VerifyGroupIndex(Position: Integer; Value: Byte);
var
  I: Integer;
begin
...
function TMenuItemKadir.GetHandle: HMENU;
begin
{$IF DEFINED(CLR)}
...
{$ELSE}
  if FHandle = 0 then
  begin
    if Owner is TPopupMenuKadir then
...
procedure TMenuItemKadir.DefineProperties(Filer: TFiler);
begin
  inherited DefineProperties(Filer);
  Filer.DefineProperty('ShortCutText', ReadShortCutText, nil, False);
end;

procedure TMenuItemKadir.DoDrawText(ACanvas: TCanvas; const ACaption: string;
  var Rect: TRect; Selected: Boolean; Flags: Longint);
var
  Text: string;
  R: TRect;
...
procedure TMenuItemKadir.DrawItem(ACanvas: TCanvas; ARect: TRect; Selected: Boolean);
begin
  if Assigned(FOnDrawItem) then
    FOnDrawItem(Self, ACanvas, ARect, Selected);
end;

procedure DoThemedDrawText(MenuItem: TMenuItemKadir; ACanvas: TCanvas;
...
procedure TMenuItemKadir.AdvancedDrawItem(ACanvas: TCanvas; ARect: TRect;
...
var
  ...
  begin
    ...
    if ParentMenu is TMenuKadir then
      Alignment := paLeft
    else if ParentMenu is TPopupMenuKadir then
      Alignment := TPopupMenuKadir(ParentMenu).Alignment
    else
      Alignment := paLeft;
...
function TMenuItemKadir.GetEnumerator: TMenuItemEnumerator;
begin
  Result := TMenuItemEnumerator.Create(Self);
end;

function TMenuItemKadir.GetImageList: TCustomImageList;
var
  LItem: TMenuItemKadir;
  LMenu: TMenuKadir;
begin
  Result := nil;
  LItem := Parent;
...
procedure TMenuItemKadir.MeasureItem(ACanvas: TCanvas; var Width, Height: Integer);
const
  Alignments: array[TPopupAlignment] of Word = (DT_LEFT, DT_RIGHT, DT_CENTER);
var
  Alignment: TPopupAlignment;
...
function TMenuItemKadir.HasParent: Boolean;
begin
  Result := True;
end;

procedure TMenuItemKadir.SetBreak(Value: TMenuBreak);
begin
...
procedure TMenuItemKadir.SetCaption(const Value: string);
begin
...
procedure TMenuItemKadir.TurnSiblingsOff;
var
  I: Integer;
  Item: TMenuItemKadir;
begin
...
procedure TMenuItemKadir.SetChecked(Value: Boolean);
begin
...
procedure TMenuItemKadir.SetEnabled(Value: Boolean);
begin
...
procedure TMenuItemKadir.SetGroupIndex(Value: Byte);
begin
...
function TMenuItemKadir.GetAction: TBasicAction;
begin
...
function TMenuItemKadir.GetActionLinkClass: TMenuActionLinkClass;
begin
  Result := TMenuActionLink;
end;

function TMenuItemKadir.GetCount: Integer;
begin
...
procedure TMenuItemKadir.SetShortCut(Value: TShortCut);
begin
...
procedure TMenuItemKadir.SetVisible(Value: Boolean);
begin
...
procedure TMenuItemKadir.SetImageIndex(Value: TImageIndex);
begin
...
function TMenuItemKadir.GetMenuIndex: Integer;
begin
  Result := -1;
  if FParent <> nil then Result := FParent.IndexOf(Self);
end;

procedure TMenuItemKadir.SetMenuIndex(Value: Integer);
var
  Parent: TMenuItemKadir;
  Count: Integer;
begin
  if FParent <> nil then
  begin
...
procedure TMenuItemKadir.GetChildren(Proc: TGetChildProc; Root: TComponent);
var
  I: Integer;
begin
...
procedure TMenuItemKadir.SetChildOrder(Child: TComponent; Order: Integer);
begin
  (Child as TMenuItemKadir).MenuIndex := Order;
end;

procedure TMenuItemKadir.SetDefault(Value: Boolean);
var
  I: Integer;
begin
...
procedure TMenuItemKadir.InitiateAction;
begin
  if FActionLink <> nil then FActionLink.Update;
end;

procedure TMenuItemKadir.Insert(Index: Integer; Item: TMenuItemKadir);
begin
  if Item.FParent <> nil then
    Error({$IFNDEF CLR}@{$ENDIF}SMenuReinserted);
  if FItems = nil then FItems := TList.Create;
  if (Index - 1 >= 0) and (Index - 1 < FItems.Count) then
    if Item.GroupIndex < TMenuItemKadir(FItems[Index - 1]).GroupIndex then
      Item.GroupIndex := TMenuItemKadir(FItems[Index - 1]).GroupIndex;
...
procedure TMenuItemKadir.Delete(Index: Integer);
var
  Cur: TMenuItemKadir;
begin
  if (Index < 0) or (FItems = nil) or (Index >= GetCount) then
    Error({$IFNDEF CLR}@{$ENDIF}SMenuIndexError);
  Cur := TMenuItemKadir(FItems[Index]);
  FItems.Delete(Index);
  Cur.FParent := nil;
  Cur.FOnChange := nil;
{$IF DEFINED(CLR)}
...
procedure TMenuItemKadir.Click;
begin
  if Enabled then
  begin
...
function TMenuItemKadir.IndexOf(Item: TMenuItemKadir): Integer;
begin
  Result := -1;
  if FItems <> nil then Result := FItems.IndexOf(Item);
end;

procedure TMenuItemKadir.Add(Item: TMenuItemKadir);
begin
  Insert(GetCount, Item);
end;

procedure TMenuItemKadir.Remove(Item: TMenuItemKadir);
var
  I: Integer;
begin
  I := IndexOf(Item);
...
procedure TMenuItemKadir.MenuChanged(Rebuild: Boolean);
var
  Source: TMenuItemKadir;
begin
  if (Parent = nil) and (Owner is TMenuKadir) then
    Source := nil else
    Source := Self;
  if Assigned(FOnChange) then FOnChange(Self, Source, Rebuild);
end;

procedure TMenuItemKadir.SubItemChanged(Sender: TObject; Source: TMenuItemKadir; Rebuild: Boolean);
begin
{$IF DEFINED(CLR)}
  if Rebuild and
...
function TMenuItemKadir.GetBitmap: TBitmap;
begin
...
procedure TMenuItemKadir.SetAction(Value: TBasicAction);
begin
  if Value = nil then
  begin
    FActionLink.Free;
...
procedure TMenuItemKadir.SetBitmap(Value: TBitmap);
begin
  if FBitmap = nil then FBitmap := TBitmap.Create;
...
procedure TMenuItemKadir.InitiateActions;
var
  I: Integer;
begin
...
[UIPermission(SecurityAction.LinkDemand, Window=UIPermissionWindow.AllWindows)]
function TMenuItemKadir.GetParentComponent: TComponent;
begin
...
[UIPermission(SecurityAction.LinkDemand, Window=UIPermissionWindow.AllWindows)]
procedure TMenuItemKadir.SetParentComponent(Value: TComponent);
begin
  if FParent <> nil then FParent.Remove(Self);
  if Value <> nil then
    if Value is TMenuKadir then
      TMenuKadir(Value).Items.Add(Self)
    else if Value is TMenuItemKadir then
      TMenuItemKadir(Value).Add(Self);
end;

[UIPermission(SecurityAction.LinkDemand, Window=UIPermissionWindow.AllWindows)]
function TMenuItemKAdir.GetParentMenu: TMenuKadir;
var
  MenuItem: TMenuItemKadir;
begin
  MenuItem := Self;
...
procedure TMenuItemKadir.SetRadioItem(Value: Boolean);
begin
...
procedure TMenuItemKadir.ActionChange(Sender: TObject; CheckDefaults: Boolean);
begin
...
procedure TMenuItemKadir.DoActionChange(Sender: TObject);
begin
  if Sender = Action then ActionChange(Sender, False);
end;

function TMenuItemKadir.IsCaptionStored: Boolean;
begin
  Result := (ActionLink = nil) or not FActionLink.IsCaptionLinked;
end;

function TMenuItemKadir.IsCheckedStored: Boolean;
begin
  Result := (ActionLink = nil) or not FActionLink.IsCheckedLinked;
end;

function TMenuItemKadir.IsEnabledStored: Boolean;
begin
  Result := (ActionLink = nil) or not FActionLink.IsEnabledLinked;
end;

function TMenuItemKadir.IsHintStored: Boolean;
begin
  Result := (ActionLink = nil) or not FActionLink.IsHintLinked;
end;

function TMenuItemKadir.IsHelpContextStored: Boolean;
begin
  Result := (ActionLink = nil) or not FActionLink.IsHelpContextLinked;
end;

function TMenuItemKadir.IsImageIndexStored: Boolean;
begin
  Result := (ActionLink = nil) or not FActionLink.IsImageIndexLinked;
end;

function TMenuItemKadir.IsShortCutStored: Boolean;
begin
  Result := (ActionLink = nil) or not FActionLink.IsShortCutLinked;
end;

function TMenuItemKadir.IsVisibleStored: Boolean;
begin
  Result := (ActionLink = nil) or not FActionLink.IsVisibleLinked;
end;

function TMenuItemKadir.IsOnClickStored: Boolean;
begin
  Result := (ActionLink = nil) or not FActionLink.IsOnExecuteLinked;
end;

procedure TMenuItemKadir.AssignTo(Dest: TPersistent);
begin
...
procedure TMenuItemKadir.Notification(AComponent: TComponent;
...
procedure TMenuItemKadir.SetSubMenuImages(Value: TCustomImageList);
begin
...
procedure TMenuItemKadir.ImageListChange(Sender: TObject);
begin
...
{$ENDIF ITERATOR_OBJECTS}
    class procedure DoIterateMenus(Menu1, Menu2: TMenuItemKadir);
  end;
...
{$ELSE !ITERATOR_OBJECTS}
function UpdateItem(MenuItem: TMenuItemKadir): Boolean;
begin
  Result := False;
...
class procedure TMenuUpdater.DoIterateMenus(Menu1, Menu2: TMenuItemKadir);
{$IFDEF ITERATOR_OBJECTS}
var
...
procedure TMenuItemKadir.UpdateItems;
begin
//  IterateMenus({$IFNDEF CLR}@{$ENDIF}UpdateItem, FMerged, Self);
  TMenuUpdater.DoIterateMenus(FMerged, Self);
end;

procedure TMenuItemKadir.Add(const AItems: array of TMenuItemKadir);
var
  I: Integer;
begin
...
procedure TMenuItemKadir.Clear;
var
  I: Integer;
begin
...
function TMenuItemKadir.InternalRethinkHotkeys(ForceRethink: Boolean): Boolean;
var
...
{$IFEND}
  ...
  begin
    ...
    for I := 0 to LDoing.Count - 1 do
      LBest.AddObject(TMenuItemKadir(LDoing.Objects[I]).FCaption, LDoing.Objects[I]);
...
{$IFEND}
        ...
        TMenuItemKadir(LBest.Objects[I]).FCaption := LCaption;
      end;
    finally
{$IF NOT DEFINED(CLR)}
      LBest.Free;
...
function TMenuItemKadir.RethinkHotkeys: Boolean;
begin
...
procedure TMenuItemKadir.SetAutoHotkeys(const Value: TMenuItemAutoFlag);
begin
...
function TMenuItemKadir.IsLine: Boolean;
begin
  Result := FCaption = cLineCaption;
end;

function TMenuItemKadir.Find(ACaption: string): TMenuItemKadir;
var
  I: Integer;
begin
  Result := nil;
...
function TMenuItemKadir.InsertNewLine(ABefore: Boolean; AItem: TMenuItemKadir): Integer;
begin
...
function TMenuItemKadir.InsertNewLineAfter(AItem: TMenuItemKadir): Integer;
begin
  Result := InsertNewLine(False, AItem);
end;

function TMenuItemKadir.InsertNewLineBefore(AItem: TMenuItemKadir): Integer;
begin
  Result := InsertNewLine(True, AItem);
end;

function TMenuItemKadir.NewBottomLine: Integer;
begin
  Result := 0;
  if Count = 0 then
    Add(NewLine)
...
function TMenuItemKadir.NewTopLine: Integer;
begin
  Result := 0;
  if Count = 0 then
    Add(NewLine)
...
function TMenuItemKadir.InternalRethinkLines(ForceRethink: Boolean): Boolean;
var
  I, LLastAt: Integer;
  LLastBar: TMenuItemKadir;
begin
  Result := False;
  if ForceRethink or
...
procedure TMenuItemKadir.SetAutoLineReduction(const Value: TMenuItemAutoFlag);
begin
...
function TMenuItemKadir.RethinkLines: Boolean;
begin
...
function TMenuItemKadir.GetAutoHotkeys: Boolean;
var
...
function TMenuItemKadir.GetAutoLineReduction: Boolean;
var
  LAuto: TMenuItemAutoFlag;
begin
...
constructor TMenuKadir.Create(AOwner: TComponent);
begin
...
{$ELSE}
  FItems := TMenuItemKadir.Create(Self);
  FItems.FOnChange := MenuChanged;
  FItems.FMenu := Self;
  FImageChangeLink := TChangeLink.Create;
  FImageChangeLink.OnChange := ImageListChange;
...
function TMenuKadir.CreateMenuItem: TMenuItemKadir;
begin
  Result := TMenuItemKadir.Create(Self);
end;

destructor TMenuKadir.Destroy;
begin
  FItems.Free;
...
[UIPermission(SecurityAction.LinkDemand, Window=UIPermissionWindow.AllWindows)]
procedure TMenuKadir.GetChildren(Proc: TGetChildProc; Root: TComponent);
begin
  FItems.GetChildren(Proc, Root);
end;

function TMenuKadir.GetHandle: HMENU;
begin
{$IF DEFINED(CLR)}
...
procedure TMenuKadir.SetChildOrder(Child: TComponent; Order: Integer);
begin
  FItems.SetChildOrder(Child, Order);
end;

procedure TMenuKadir.UpdateItems;
begin
...
{$ELSE !ITERATOR_OBJECTS}
function TMenuKadir.FindItem(Value: NativeUInt; Kind: TFindItemKind): TMenuItemKadir;
var
  FoundItem: TMenuItemKadir;

  function Find(Item: TMenuItemKadir): Boolean;
  var
    I: Integer;
  begin
    Result := False;
...
function TMenuKadir.GetHelpContext(Value: NativeUInt; ByCommand: Boolean): THelpContext;
var
  Item: TMenuItemKadir;
  Kind: TFindItemKind;
begin
  ...
  if (Kind = fkHandle) and (Self is TPopupMenuKadir) and
    (UIntPtr(TPopupMenuKadir(Self).Handle) = Value) then
    Result := TPopupMenuKadir(Self).HelpContext
  else
  begin
...
function TMenuKadir.DispatchCommand(ACommand: Word): Boolean;
var
  Item: TMenuItemKadir;
begin
  Result := False;
...
function TMenuKadir.DispatchPopup(AHandle: HMENU): Boolean;
var
  Item: TMenuItemKadir;
  LRebuild: Boolean;
begin
  ...
  else if not (csDesigning in ComponentState) and (Self is TPopupMenuKadir) then
    Items.InitiateActions;
end;

function TMenuKadir.IsOwnerDraw: Boolean;
begin
...
[UIPermission(SecurityAction.LinkDemand, Window=UIPermissionWindow.AllWindows)]
function TMenuKadir.IsShortCut(var Message: TWMKey): Boolean;
type
...
{$ELSE}
  MyItems: TMenuItemKadir;
  ShortCutItem: TMenuItemKadir;
{$IFEND}

  function NthParentOf(Item: TMenuItemKAdir; N: Integer): TMenuItemKadir;
  begin
    Result := Item;
  ...
  function DoClick(var Item: TMenuItemKadir; Level: Integer): TClickResult;
  var
    ItemParent: TMenuItemKadir;
  begin
    Result := crClicked;
...
function TMenuKadir.IsBiDiModeStored: Boolean;
begin
  Result := not FParentBiDiMode;
end;

procedure TMenuKadir.DoBiDiModeChanged;
const
  BufSize = 80;
var
  Menu: HMENU;
...
{$ELSE}
function TMenuKadir.UpdateImage: Boolean;
var
...
procedure TMenuKadir.SetOwnerDraw(Value: Boolean);
begin
...
procedure TMenuKadir.AdjustBiDiBehavior;
var
  SaveBiDi: TBiDiMode;
...
procedure TMenuKadir.SetWindowHandle(Value: HWND);
begin
  FWindowHandle := Value;
  UpdateImage;
...
procedure TMenuKadir.DoChange(Source: TMenuItemKadir; Rebuild: Boolean);
begin
  if Assigned(FOnChange) then FOnChange(Self, Source, Rebuild);
end;

procedure TMenuKadir.Loaded;
begin
  inherited Loaded;
  DoChange(nil, False);
end;

procedure TMenuKadir.MenuChanged(Sender: TObject; Source: TMenuItemKadir; Rebuild: Boolean);
begin
  if ComponentState * [csLoading, csDestroying] = [] then DoChange(Source, Rebuild);
end;

procedure TMenuKadir.ImageListChange(Sender: TObject);
begin
  if Sender = Images then UpdateItems;
end;

procedure TMenuKadir.SetImages(Value: TCustomImageList);
begin
...
procedure TMenuKadir.Notification(AComponent: TComponent;
  Operation: TOperation);
begin
  inherited Notification(AComponent, Operation);
...
function TMenuKadir.IsRightToLeft: Boolean;
begin
...
{$ELSE}
procedure TMenuKadir.ProcessMenuChar(var Message: TWMMenuChar);
{$IFEND}
var
...
{$ELSE}
function TMenuKadir.DoGetMenuString(Menu: HMENU; ItemID: UINT; Str: PChar;
  MaxCount: Integer; Flag: UINT): Integer;
{$IFEND}
var
  Item: TMenuItemKadir;
...
procedure TMenuKadir.SetBiDiMode(Value: TBiDiMode);
begin
...
procedure TMenuKadir.SetParentBiDiMode(Value: Boolean);
begin
...
procedure TMenuKadir.ParentBiDiModeChanged;
var
  AForm: TWinControl;
begin
  if FParentBiDiMode then
...
procedure TMenuKadir.ParentBiDiModeChanged(AControl: TObject);
begin
  if FParentBiDiMode then
  begin
...
function TMenuKadir.GetAutoHotkeys: TMenuAutoFlag;
begin
  Result := cItemAutoFlagToMenu[Items.FAutoHotkeys];
end;

procedure TMenuKadir.SetAutoHotkeys(const Value: TMenuAutoFlag);
begin
  Items.FAutoHotkeys := cMenuAutoFlagToItem[Value];
end;

function TMenuKadir.GetAutoLineReduction: TMenuAutoFlag;
begin
  Result := cItemAutoFlagToMenu[Items.FAutoLineReduction];
end;

procedure TMenuKadir.SetAutoLineReduction(const Value: TMenuAutoFlag);
begin
...
procedure TMainMenu.MenuChanged(Sender: TObject; Source: TMenuItemKadir; Rebuild: Boolean);
var
  NeedUpdate: Boolean;
begin
  if (FWindowHandle <> 0) then
...
var
  ...
  procedure ProcessAccels(Item: TMenuItemKadir);
  var
    I: Integer;
    Virt: Byte;
  begin
  ...
  function ProcessAccelItems(Item: TMenuItemKadir): Boolean;
  var
    I: Integer;
  begin
    for I := 0 to High(Groups) do
...
var
  ...
  function AddOle2(Item: TMenuItemKadir): Boolean;
  var
    I: Integer;
  begin
...
begin
  case Message.Msg of
    WM_COMMAND:
      for I := 0 to Count - 1 do
        if TPopupMenuKadir(Items[I]).DispatchCommand(Message.wParam) then Exit;
    WM_INITMENUPOPUP:
      with TWMInitMenuPopup(Message) do
        for I := 0 to Self.Count - 1 do
          if TPopupMenuKadir(Items[I]).DispatchPopup(MenuPopup) then Exit;
    WM_MENUSELECT:
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            Item := IDItem;
          MenuItem := TPopupMenuKadir(Items[I]).FindItem(Item, FindKind);
...
{$IFEND}
      begin
        for I := 0 to Count - 1 do
        begin
          if HMENU(hItemHandle) = TMenuKadir(Items[I]).Handle then
            MenuItem := TMenuKadir(Items[I]).Items
          else
            MenuItem := TPopupMenuKadir(Items[I]).FindItem(hItemHandle, fkHandle);
          if MenuItem <> nil then
          begin
            ContextID := TMenuKadir(Items[I]).GetHelpContext(iCtrlID, True);
            if ContextID = 0 then
              ContextID := TMenuKadir(Items[I]).GetHelpContext(hItemHandle, False);
...
{$IFEND}
      begin
        for I := 0 to Count - 1 do
        begin
          MenuItem := TPopupMenuKadir(Items[I]).FindItem(itemID, fkCommand);
          if MenuItem <> nil then
          begin
...
{$IFEND}
    ...
    WM_MENUCHAR:
      for I := 0 to Count - 1 do
        with TPopupMenuKadir(Items[I]) do
...
procedure TPopupList.Add(Popup: TPopupMenuKadir);
begin
  if Count = 0 then FWindow := AllocateHWnd(MainWndProc);
  inherited Add(Popup);
end;

procedure TPopupList.Remove(Popup: TPopupMenuKadir);
begin
...
constructor TPopupMenuKadir.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  FPopupPoint.X := -1;
  FPopupPoint.Y := -1;
...
destructor TPopupMenuKadir.Destroy;
begin
  PopupList.Remove(Self);
  inherited Destroy;
end;

procedure TPopupMenuKadir.CloseMenu;
begin
  Winapi.Windows.EndMenu;
end;

procedure TPopupMenuKadir.DoPopup(Sender: TObject);
begin
  if Assigned(FOnPopup) then FOnPopup(Sender);
end;

function TPopupMenuKadir.GetHelpContext: THelpContext;
begin
  Result := FItems.HelpContext;
end;

procedure TPopupMenuKadir.SetHelpContext(Value: THelpContext);
begin
  FItems.HelpContext := Value;
end;

procedure TPopupMenuKadir.SetBiDiModeFromPopupControl;
var
  AControl: TControl;
begin
  if not SysLocale.MiddleEast then Exit;
...
function TPopupMenuKadir.UseRightToLeftAlignment: Boolean;
var
  AControl: TControl;
begin
  Result := False;
...
procedure TPopupMenuKadir.Popup(X, Y: Integer);
const
...
procedure TPopupMenuKadir.SetPopupPoint(APopupPoint: TPoint);
begin
...
procedure TMenuItemStack.ClearItem(AItem: TMenuItemKadir);
var
  I: Integer;
begin
...
procedure InitMenuItems(AMenu: TMenuKadir; const Items: array of TMenuItemKadir);
var
  I: Integer;

  procedure SetOwner(Item: TMenuItemKadir);
  var
    I: Integer;
  begin
...
function NewPopupMenu(Owner: TComponent; const AName: string;
  Alignment: TPopupAlignment; AutoPopup: Boolean; const Items: array of TMenuItemKadir): TPopupMenuKadir;
begin
  Result := TPopupMenuKadir.Create(Owner);
  Result.Name := AName;
...
function NewSubMenu(const ACaption: string; hCtx: THelpContext;
  const AName: string; const Items: array of TMenuItemKadir; AEnabled: Boolean): TMenuItemKadir;
var
  I: Integer;
begin
  Result := TMenuItemKadir.Create(nil);
  for I := Low(Items) to High(Items) do
    Result.Add(Items[I]);
...
{$IF NOT DEFINED(CLR)}
  ...
  GroupDescendentsWith(TMenuItemKadir, TControl);
  GroupDescendentsWith(TMenuKadir, TControl);
{$IFEND}
  RegisterClass(TMenuItemKadir);
